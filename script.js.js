...
$("checkoutBtn").onclick = () => {
  ...
  if (!t.ids.length) return toast("Your cart is empty");
  if (!selection.theatre || !selection.movie || !selection.show || !selection.seat) return toast("Complete your theatre, show and seat selection first");
  if (renderCheckout()) navigate("payment");
};
...
function renderCheckout() {
  const t = getCartTotals();
  if (!selection.theatre || !selection.movie || !selection.show || !selection.seat || !t.ids.length) {
    $("checkoutSummary").innerHTML = `<p class="muted">Your checkout details are incomplete. Please return to the menu and finish your selection.</p>`;
    $("deliveryDetails").innerHTML = "";
    $("placeOrderAmount").textContent = money(0);
    return false;
  }
  $("deliveryDetails").innerHTML = `<div class="delivery-icon">💺</div><div><b>Seat ${selection.seat}</b><p>${selection.movie.name} · ${selection.show}</p><small>${selection.theatre.name} · ${selection.location}</small></div>`;
  $("checkoutSummary").innerHTML = `<div class="summary-items">${t.ids.map(id => { const item=MENU.find(x=>x.id===id); return `<div class="summary-item"><span class="summary-emoji">${item.emoji}</span><div><b>${item.name}</b><small>Qty ${cart[id]}</small></div><strong>${money(item.price*cart[id])}</strong></div>`; }).join("")}</div><div class="summary-total"><div><span>Subtotal</span><b>${money(t.subtotal)}</b></div><div><span>Service fee</span><b>${money(t.fee)}</b></div><div class="final-total"><span>Total</span><strong>${money(t.total)}</strong></div></div>`;
  $("placeOrderAmount").textContent = money(t.total);
  return true;
}

document.querySelectorAll('input[name="payment"]').forEach(r => r.onchange = () => {
  if (r.checked) {
    $("upiBox").classList.toggle("hidden", r.value !== "upi");
...
$("placeOrderBtn").onclick = () => {
  ...
  const t = getCartTotals();
  if (!t.ids.length) return $("paymentMessage").textContent = "Your cart is empty.";
  if (!selection.theatre || !selection.movie || !selection.show || !selection.seat) return $("paymentMessage").textContent = "Please complete your show and seat selection.";
  ...
  $("placeOrderBtn").firstChild.textContent = "Placing Order… ";
